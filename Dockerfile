...
# Ensure the developer user's home directory and /workspaces/aws-sqs-teardown-queue are owned by him/her:
# (A workaround to a side effect of setting WORKDIR before creating the user)
RUN mkdir -p /workspaces/aws-sqs-teardown-queue && chown -R ${DEVELOPER_UID}:node /workspaces/aws-sqs-teardown-queue

# Add the project's executable path to the system PATH:
ENV PATH=/workspaces/aws-sqs-teardown-queue/bin:$PATH

# Configure the app dir as the working dir:
WORKDIR /workspaces/aws-sqs-teardown-queue

# Switch to the developer user:
USER ${DEVELOPER_UID}

# Copy and install the project dependency lists into the container image:
COPY --chown=${DEVELOPER_UID} package.json yarn.lock /workspaces/aws-sqs-teardown-queue/
RUN yarn install
ENV PATH=/workspaces/aws-sqs-teardown-queue/node_modules/.bin:$PATH
...
COPY --chown=${DEVELOPER_UID} . /workspaces/aws-sqs-teardown-queue/
RUN yarn build
...
COPY --from=builder --chown=node:node /workspaces/aws-sqs-teardown-queue /workspaces/aws-sqs-teardown-queue
WORKDIR /workspaces/aws-sqs-teardown-queue
